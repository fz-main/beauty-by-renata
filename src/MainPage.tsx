    ...
    <div className="w-screen h-screen bg-[#0a0a0a] text-[#f8f5f2] overflow-hidden relative selection:bg-[#e5d3b3] selection:text-black">
      ...
      <div className="absolute inset-0 z-10 pointer-events-none">
        ...
        <AnimatePresence mode="wait">
          ...
          {stage === STAGES.ABOUT && (<motion.div key="about" initial={{ opacity: 0, y: 60 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 60 }} transition={{ duration: 1, ease: 'easeOut' }} className="absolute inset-0 pointer-events-auto overflow-y-auto flex flex-col px-6 py-20"><button onClick={() => setStage(STAGES.MENU)} className="fixed top-16 md:top-20 left-4 md:left-8 font-monument text-[10px] md:text-xs tracking-widest hover:text-[#e5d3b3] transition-colors z-50 flex items-center gap-3 group bg-black/60 px-3 py-2 rounded-full backdrop-blur-sm"><span className="w-4 h-[1px] bg-white group-hover:bg-[#e5d3b3] transition-colors" />{t.back}</button><div id="about-scroll-container" className="max-w-5xl w-full mx-auto"><div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-start"><motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1.2, delay: 0.2, ease: 'easeOut' }} className="flex justify-center cursor-pointer" onClick={() => setLightboxImage(t.aboutPhoto)}><img src={t.aboutPhoto} alt="Renata Birjukov" className="w-full max-w-md object-cover rounded-3xl shadow-2xl transition-transform hover:scale-105" /></motion.div><motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1.2, delay: 0.4, ease: 'easeOut' }}><div className="font-monument text-[9px] tracking-[0.3em] text-[#e5d3b3] mb-4 uppercase">{t.aboutLabel}</div><h2 className="font-editorial text-4xl md:text-5xl mb-2 leading-tight">{t.ownerName}</h2><div className="font-montreal text-xs text-[#a3a3a3] tracking-widest mb-6">{t.aboutFounder}</div><div className="border-t border-white/10 pt-6 flex flex-col gap-4"><p className="font-montreal text-sm text-[#a3a3a3] leading-relaxed">{t.aboutBio}</p><p className="font-montreal text-sm leading-relaxed" style={{ color: '#e5d3b3' }}>{t.aboutMotto}</p><div className="font-monument text-[9px] tracking-widest text-[#a3a3a3] mt-2">{t.aboutServices}</div></div></motion.div></div><Testimonials lang={lang} t={t} /><HelixGallery t={t} />
                <div className="w-full mt-12 glass-panel rounded-3xl p-6 md:p-10 text-center">
                  <div className="font-monument text-[10px] md:text-xs tracking-[0.25em] text-[#e5d3b3] mb-4">Dárkové poukazy</div>
                  <h3 className="font-editorial text-2xl md:text-4xl mb-4">Darujte zážitek a péči</h3>
                  <p className="font-montreal text-sm md:text-base text-white/60 leading-relaxed max-w-xl mx-auto mb-6">Potěšte své blízké dárkovým poukazem na profesionální kosmetické ošetření. Ideální dárek k narozeninám, Vánocům nebo jen tak pro radost. Poukaz platí 6 měsíců od zakoupení.</p>
                  <div className="flex flex-wrap justify-center gap-4">
                    <a href={t.contacts?.instagram} target="_blank" rel="noopener noreferrer" className="px-6 py-3 bg-white text-black font-monument text-[10px] tracking-widest rounded-full hover:bg-[#e5d3b3] transition-colors">Napsat o poukazu</a>
                    <a href={`tel:${t.contacts?.phone?.replace(/\s/g, '')}`} className="px-6 py-3 border border-white/30 text-white font-monument text-[10px] tracking-widest rounded-full hover:bg-white/10 transition-colors">Zavolat</a>
                  </div>
